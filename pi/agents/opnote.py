...
class OpNoteAgent(ProjectionAgent):
    ...
    def build_system(self, profile: SiteProfile) -> str:
        ...
        setting = profile.care_setting.replace("_", " ")
        return (
            f"You draft a brief {setting} record from the reconstructed timeline. "
            ...
            "clinician review - do not invent detail that is not in the timeline."
            f"{gloss}"
        )
